...
for i in range(len(particle_types)):
    ...
    for j in range(i, len(particle_types)):
        ...
        yukawa.pair_coeff.set(aa_i, aa_j, kappa=0.1*10, epsilon=qiqj)
...
h_e = hoomd.variant.linear_interp([($contract_t, $final_slab_x), ($slab_t, $final_slab_z)])
...
for i in range(len(particle_types)):
    ...
    lang.set_gamma(aa_i, mass/10)
    lang.set_gamma_r(aa_i, mass/10)
