...
particles, particle_types = $particles, $particle_types
protein = "$protein"
chains = $chains
sequence = "$sequence"
topo_path = "$topo_path"
...
for i in range(len(particle_types)):
    ...
    for j in range(i, len(particle_types)):
        ...
        if aa_i in pairs and aa_j in pairs and aa_i != aa_j:
            print("Doing", aa_i, aa_j)
            hps_table.pair_coeff.set(aa_i, aa_j, func=HPS_pi_potential,
                                     rmin=0.2,
                                     rmax=3*sigma/10,
                                    coeff = dict(eps_hps=hps_lj, eps_pi=hps_lj_cat, lambd=lambd, sigma=sigma / 10))
