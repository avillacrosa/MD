...
class LMP(analysis.Analysis):
    ...
    """
    def __init__(self, md_dir, force_reorder=False, equil_frames=300, silent=False, low_mem=False, **kwargs):
        self.md_dir = md_dir
        ...
        self.temper = None
        self.every_frames, self.last_frames = kwargs.get('every', None), None
    ...
    def save_movies(self, frames=100, T=None, center=True):
        ...
        for T, struct in enumerate(structures):
            ...
            self.save_lammpstrj(small_struct, fname=(os.path.join(self.md_dir, f'{self.protein}x{self.chains}-T{self.temperatures[T]:.0f}.lammpstrj')), center=center)
            small_struct.save_xtc(os.path.join(self.md_dir, f'{self.protein}x{self.chains}-T{self.temperatures[T]:.0f}.xtc'))
            small_struct.save_netcdf(os.path.join(self.md_dir, f'{self.protein}x{self.chains}-T{self.temperatures[T]:.0f}.netcdf'))
    ...
    def get_structures(self, total_frames=1000, every=1):
        ...
        if total_frames is not None and not self.rerun and not self.silent:
            print(f"> Taking frames every {self.every_frames} for a total of {n_frames} to avoid strong correlations")
        return structures
