...
class fgRPA:
    ...
    def ds(self):
        ...
        # s = log(phi_m)/N + 1 / N + dphi_c + dphi_c*log(phi_c)
        s = log(phi_m)/N + 1 / N + dphi_c + dphi_c*log(phi_c) + dphi_w + dphi_w*log(phi_w)
        s = -s
        return s
    ...
    def dGk(self, k):
        ...
        for i, sigma_i in enumerate(self.sigmas):
            for j, sigma_j in enumerate(self.sigmas):
                ...
                Gm += sigma_i * gauss * sigma_j
        # Eq. 40
        G = 4*pi*(dphi_c + Gm/N)/(k**2*(1+k**2)*self.T_star)
        return G
    ...
    def dfel(self):
        ...
        def integrand(k):
            Gk = self.Gk(k)
            dGk = self.dGk(k)
            int = (1/(1+eta*Gk)*(eta*dGk))/eta-dGk
            int *= k**2/(4*pi**2)
            return int
    ...
    def find_binodals(self):
        ...
        res = sco.minimize(f+1.23465*self.phi_m)
        return res
