...
def x_eff(HP, phi, phis, u):
    print(phi, phis, u)
    print(HP['N'])
    return sco.brenth(x_to_solve, 1 / HP['N'] / 10, 100 * HP['N'], args=(HP, phi, phis, u))


def x_to_solve(x, HP, phi, phis, u):
    # print(sci.quad(x_RG_to_int, 0, np.inf, args=(HP, phi, phis, u, x), limit=intMax)[0])
    return 1 - 1 / x - sci.quad(x_RG_to_int, 0, np.inf, args=(HP, phi, phis, u, x), limit=intMax)[0] / (18 * (HP['N'] - 1))
    # return 1 - 1 / x
